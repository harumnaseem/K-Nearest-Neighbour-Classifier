...
Original file is located at
    https://colab.research.google.com/drive/1BQuFvsCC7lFxYrfiB633yvcGrEDoOka4
...
def compute_distances_no_loops(x_train, x_test):
  ...
  Flattened_x_test = torch.flatten(x_test, start_dim = 1)
  Flattened_x_train = torch.flatten(x_train, start_dim = 1)

  product = torch.mm(Flattened_x_train, Flattened_x_test.t())
  product = -2*product
  Flattened_x_test = torch.pow(Flattened_x_test, 2)
  Flattened_x_train = torch.pow(Flattened_x_train, 2)
  SumTest = torch.sum(Flattened_x_test, dim = 1)
  SumTrain = torch.sum(Flattened_x_train, dim = 1, keepdim = True)

  final = product + SumTest + SumTrain
  dists = final
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def predict_labels(dists, y_train, k=1):
  ...
  # Replace "pass" statement with your code
  for x in range(num_test):
    Labels, Indices = torch.topk(dists[:,x], k, largest = False)
    closest_y_train = []
    closest_y_train = y_train[Indices]
    Ties = torch.bincount(closest_y_train)

    y_pred[x] = torch.argmax(Ties)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
